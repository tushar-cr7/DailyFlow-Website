...
function Logo({ small = false }: { small?: boolean }) {
  return <span className={small ? 'logo-lockup logo-lockup-small' : 'logo-lockup'}><img src="/icon.svg" alt="" /><span><b>Daily<span>Flow</span></b>{!small && <small>FLOW. FOCUS. FINISH.</small>}</span></span>
}
...
function Nav() {
  ...
  return <header className="site-nav"><a href="#top"><Logo /></a><nav className={open ? 'nav-links open' : 'nav-links'}><a href="#product">Product</a><a href="#flow">The flow</a><a href="#environments">Environments</a><a href={GITHUB_URL}>GitHub</a><a className="nav-download" href={DOWNLOAD_URL}><Download size={13} /> Download</a></nav><button className="menu-toggle" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button></header>
}
